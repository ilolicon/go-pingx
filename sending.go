...
func (pinger *Pinger) SetMark(mark int) error {
	if 0 >= mark {
		return nil
	}

	if runtime.GOOS == "linux" {
		if fd := pinger.reflectConnFD(); fd >= 0 {
			return syscall.SetsockoptInt(fd, syscall.SOL_SOCKET, syscall.SO_MARK, mark)
		}
	}
	return errors.New("not implemented on " + runtime.GOOS + "/" + runtime.GOARCH)
}

// FIXME: brittle code
func (pinger *Pinger) reflectConnFD() (fd int) {
	defer func() {
	...
	if pinger.conn4.IPv4PacketConn() != nil {
		packetConn = reflect.ValueOf(pinger.conn4.IPv4PacketConn().PacketConn)
	} else if pinger.conn6.IPv6PacketConn() != nil {
		packetConn = reflect.ValueOf(pinger.conn6.IPv6PacketConn().PacketConn)
	} else {
		return -1
	}

	netfd := reflect.Indirect(packetConn).FieldByName("fd")
